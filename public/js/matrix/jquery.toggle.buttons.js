! function($) {
    ...
    $.fn.toggleButtons = function(method) {
        var $element, $div, $cb, transitionSpeed = 0.05,
            methods = {
                init: function(opt) {
                    this.each(function() {
                        var $spanLeft, $spanRight, options, moving, dataAttribute = {};
                        ...
                        $spanLeft = $('').addClass("labelLeft").text(options.label.enabled === undefined ? "ON" : options.label.enabled);
                        $spanRight = $('').addClass("labelRight").text(options.label.disabled === undefined ? "OFF " : options.label.disabled);
                        ...
                        $div = $cb.wrap($('')).parent();
                        $div.append($spanLeft);
                        $div.append($('').attr('for', $cb.attr('id') || ''));
                        ...
                        $element.find('input:checkbox').on('change', function(e, skipOnChange) {
                            ...
                            $element.stop().animate({
                                'left': active ? '0' : '-50%'
                            }, $toggleButton.data('transitionSpeed'));
